...
if($status=='1'){
			$status_query="UPDATE `care_master_NGTK_group` SET `status`='2' WHERE `slno`='$slno' ";
			$msg->success('SuccessFully Inactive');
		}else{
			$status_query="UPDATE `care_master_NGTK_group` SET `status`='1' WHERE `slno`='$slno' ";
			$msg->success('SuccessFully Active');
		}	
		$sql_exe=mysqli_query($conn,$status_query);
		header('Location:admin_ngtk_view_ngtk.php');
		exit;
}else{
