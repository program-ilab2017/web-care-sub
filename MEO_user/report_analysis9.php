...
if($_SESSION['meo_user']){
 ...
 $title="Average number of female participants (Total) in the training in the reported month";
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        ...
        <small>Average number of female participants (Total) in the training in the reported month</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active"><a href="#">New Report </a></li>
        <li class="active">Report/Analysis 9</li>
      </ol>
    </section>
                  ...
                  if(!empty($months)){                      
                    ?>
                      ...
                      <tbody>
                        <?php
                          ...
                          while($result_village=mysqli_fetch_assoc($sql_get_village)){
                            ...
                            $get_datas="SELECT AVG(`care_EV_male_Participants`),AVG(`care_EV_female_Participants`),SUM(`care_EV_male_Participants`),Sum(`care_EV_female_Participants`),COUNT(*) FROM `care_master_mrf_exposure_visit_tarina` WHERE`care_EV_month`='$months' AND `care_EV_year`='$Year' and `care_EV_vlg_name`='$care_vlg_name1' and `care_EV_district_name`='$location_user'";
                     ...
                     <table class="table table-striped table-bordered" cellspacing="0" >
                      ...
                      <tbody>
                        <tr>
                          <th>Average number of female participants (Total) </th>
                          <td>
                              <?php
                              $get_data="SELECT AVG(`care_EV_male_Participants`),AVG(`care_EV_female_Participants`),SUM(`care_EV_male_Participants`),Sum(`care_EV_female_Participants`),COUNT(*) FROM `care_master_mrf_exposure_visit_tarina` WHERE`care_EV_month`='$months' AND `care_EV_year`='$Year' and `care_EV_district_name`='$location_user'";
                                ...
                                if(!empty($fetch_result[1])){
                                   echo "(".$total_female.")/".$total_count."=";
                                  echo $total=$fetch_result[1];
                                }else{
                                  echo 0;
                                }
                 ...
                 google.charts.load('current', {'packages':['bar']});
                  ...
                  function drawStuff() {
                      ...
                      var data = google.visualization.arrayToDataTable([
                       ['village',  'Female'],
                        <?php 
                        $get_data_sc="SELECT AVG(`care_EV_male_Participants`),AVG(`care_EV_female_Participants`),SUM(`care_EV_male_Participants`),Sum(`care_EV_female_Participants`),COUNT(*) FROM `care_master_mrf_exposure_visit_tarina` WHERE`care_EV_month`='$months' AND `care_EV_year`='$Year' and `care_EV_district_name`='$location_user'";
                        ...
                        $get_village_sc="SELECT * FROM `care_master_village_info`  WHERE `care_vlg_district`='$location_user' ";
                          ...
                          while($result_village_sc=mysqli_fetch_assoc($sql_get_village_sc)){
                            ...
                            $get_datas_sc="SELECT AVG(`care_EV_male_Participants`),AVG(`care_EV_female_Participants`),SUM(`care_EV_male_Participants`),Sum(`care_EV_female_Participants`),COUNT(*)  FROM `care_master_mrf_exposure_visit_tarina` WHERE`care_EV_month`='$months' AND `care_EV_year`='$Year' and `care_EV_vlg_name`='$care_vlg_name1_sc' and `care_EV_district_name`='$location_user'";
                            ...
                            $fetch_results_sc=mysqli_fetch_row($sql_get_datas_sc);
                                ...
                                if(!empty($fetch_results_sc[0])){
                                  echo "['".strtoupper($care_vlg_name=$result_village_sc[care_vlg_name])."',".$fetch_results_sc[3]."],";
                                }
