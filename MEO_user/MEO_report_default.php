...
<label for="District_name">District :</label>
  <input type="hidden" name="form_type" id='form_type' value="<?=web_encryptIt('get_hhi_infomation')?>">
    <select class="form-control" onchange="get_block()" id="District_name" name="District_name" required="">
      ...
      <?php 
      $query="SELECT * FROM `care_master_district_info` WHERE `care_dis_status`='1' and `care_dis_name`='$location_user'";
    ...
    <?php if($block_name==""){?>
                  <select  onchange="get_gp()" class="form-control" id="block_name" name="block_name" required="">
        ...
        <label  for="gp_name">GP Name </label> 
         <?php if($gp_name==""){?>
                  <select onchange="get_village()" class="form-control" id="gp_name" name="gp_name" required="">
                    <option value="">--Select GP Name--</option>
                  </select>
                  <?php }else{?>
                    <select onchange="get_village()" class="form-control" id="gp_name" name="gp_name" required="">
                    
                   <?php $query_gp="SELECT * FROM `care_master_gp_info` WHERE `care_gp_block`='$block_name' and `care_gp_status`='1' ";
...
<script type="text/javascript">
  function get_block() {
    ...
    if(District_name!=""){
      $.ajax({
        type:'POST',
        url:'MEO_report_get_information.php',
