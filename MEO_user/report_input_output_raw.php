                        ...
                        <td><?php
                              ...
                              $care_mt_status_form1=unserialize($sql_get_detail_fetch['care_mt_status_form1']);
                                  if(!empty($form1)){
                                    ...
                                    if($Input_out>2){
                                      ?>
                                      <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('12')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x?></a>
                                      <?php
                                    }else{?>
                                      <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('12')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x?></a>
                                      <?php
