...
if($_SESSION['meo_user']){
 ...
 $title="Average quantity of input received per KG";
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Report/Analysis 21</li>
      </ol>
    </section>
                  ...
                  if(!empty($months)){                      
                    ?>
                     <table class="table table-striped table-bordered" cellspacing="0" >
                      <thead>
                        <tr id="filters">                         
                          <th>Average quantity of seed </th>
                          <th>Average quantity of Fertiliser</th>
                          <th>Average quantity of Pesticides</th>
                          <th>Average quantity of Other</th>
                        </tr>
                      </thead>
                      <tbody>
                        <?php

                               $get_data="SELECT AVG(`care_QR_seed`),COUNT(*),SUM(`care_QR_seed`) FROM `care_master_crop_diversification_crp` WHERE `care_CRP_month`='$months' and `care_CRP_year`='$Year' and `care_CRP_vlg_name`='$village' and `care_form_type`='2' and `care_IR_seed`='1'  ";
                                $sql_get_data=mysqli_query($conn,$get_data);
                                $fetch_result=mysqli_fetch_row($sql_get_data);

                                $get_data1="SELECT  AVG(`care_QR_fertiliser`),COUNT(*),SUM(`care_QR_fertiliser`)  FROM `care_master_crop_diversification_crp` WHERE `care_CRP_month`='$months' and `care_CRP_year`='$Year' and `care_CRP_vlg_name`='$village' and `care_form_type`='2' and `care_IR_fertiliser`='1' ";
                                $sql_get_data1=mysqli_query($conn,$get_data1);
                                $fetch_result1=mysqli_fetch_row($sql_get_data1);

                                $get_data2="SELECT  AVG(`care_QR_pesticides`) ,COUNT(*),SUM(`care_QR_pesticides`)  FROM `care_master_crop_diversification_crp` WHERE `care_CRP_month`='$months' and `care_CRP_year`='$Year' and `care_CRP_vlg_name`='$village' and `care_form_type`='2' and `care_IR_pesticides`='1' ";
                                $sql_get_data2=mysqli_query($conn,$get_data2);
                                $fetch_result2=mysqli_fetch_row($sql_get_data2);

                                 $get_data3="SELECT  AVG(`care_QR_other`) ,COUNT(*),SUM(`care_QR_other`)  FROM `care_master_crop_diversification_crp` WHERE `care_CRP_month`='$months' and `care_CRP_year`='$Year' and `care_CRP_vlg_name`='$village' and `care_form_type`='2' and `care_IR_other`='1' ";
                                $sql_get_data3=mysqli_query($conn,$get_data3);
                                $fetch_result3=mysqli_fetch_row($sql_get_data3);
                        ...
                        <tr>
                          ...
                          <td>
                            <?php
                            if(!empty($fetch_result1[0])){
                               echo round($fetch_result1[0],2);
                            }else{
                              echo "0";
                            }
                          ...
                          <td>
                            <?php
                            if(!empty($fetch_result2[0])){
                              echo round($fetch_result2[0],2);
                            }else{
                              echo "0";
                            }
                          ...
                          <td>
                            <?php
                            if(!empty($fetch_result3[0])){
                              echo round($fetch_result3[0],2);
                            }else{
                              echo "0";
                            }
