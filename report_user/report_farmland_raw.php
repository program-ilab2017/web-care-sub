                   ...
                   <table id="example" class="display" cellspacing="0" width="100%">
                    <thead>
                      <tr>
                        ...
                        <?php  if(($form_type==0) ||($form_type==1) ){?>
                        ...
                        <td><?php
                              ...
                              if(!empty($form7)){
                               ...
                               $x7=count($form7);
                                if($Input_out6!="2"){
                                  ?>
                                   <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('1')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x7?></a>
                              ...
                              $care_mt_status_form8=unserialize($sql_get_detail_fetch['care_mt_status_form8']);
                                  if(!empty($form8)){
                                   for ($i=0; $i <count($form8) ; $i++) {
                                    ...
                                    if($Input_out7!="2"){
                                     ?>
                                      <a class="btn btn-danger btn-xs" href="report_MT_go_form_go.php?form_type=<?=web_encryptIt('ilab')?>&form_id=<?=web_encryptIt('4')?>&care_hhi=<?=web_encryptIt($care_hhi_id_info)?>&target=<?=web_encryptIt($months)?>&year=<?=web_encryptIt($Year)?>&village=<?=web_encryptIt($village)?>&form_uses=2" >click to View <?=$x8?></a>
